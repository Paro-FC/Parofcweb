...
function Crest({ size = "md" }) {
  ...
  return (
    <div
      className={`${sizes[size]} shrink-0 rounded-full bg-gradient-to-br from-[#ffad3d] via-yellow-500 to-[#ce0505] p-[2px] shadow-lg`}
    >
...
function FormBadge({ v }) {
  ...
  return (
    <span
      className={`grid h-[18px] w-[18px] place-items-center rounded-[4px] text-3xs font-black text-white ${styles}`}
    >
      {v}
    </span>
  );
  ...
  return (
    <div className="flex items-stretch">
      <div className="px-4 py-1 text-center">
        <div className="text-4xl font-black tabular-nums text-[#ffad3d]">
          {value}
        </div>
      ...
      {showDivider && (
        <div className="my-4 w-px bg-gradient-to-b from-transparent via-[#ffad3d]/40 to-transparent" />
      )}
    </div>
  );
}
...
function SectionCard({ children, className = "" }) {
  return (
    <div
      className={`rounded-lg border border-[#ffad3d]/20 bg-[#111111] ${className}`}
    >
      {children}
    </div>
  );
    ...
    <main className="min-h-screen bg-[#0a0a0a] font-sans text-white">
      ...
      <header className="sticky top-0 z-50 bg-transparent backdrop-blur-xl">
        <div className="mx-auto flex max-w-[1400px] items-center justify-between px-5 py-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div
                className="text-2xl font-black tracking-tight text-[#ffad3d]"
                style={{ fontFamily: "serif" }}
              >
                PARO FC
              </div>
              <div className="text-2xs font-bold uppercase tracking-[0.3em] text-white/50">
                Pride of Paro
              </div>
            </div>
          </div>
          <nav className="hidden items-center gap-6 text-xs font-bold uppercase tracking-wider lg:flex">
            <a className="text-[#ffad3d]" href="#">
              Home
            </a>
            {["Club", "Team", "Match Center"].map((item) => (
              <a
                key={item}
                className="flex items-center gap-1 text-white/70 transition hover:text-[#ffad3d]"
                href="#"
              >
                {item}{" "}
                <HugeiconsIcon
            ...
            <a
              className="text-white/70 transition hover:text-[#ffad3d]"
              href="#"
            >
              Paro FC TV
            </a>
          ...
          <div className="flex items-center gap-4 text-white/60">
            <HugeiconsIcon
              ...
              size={18}
              primaryColor="currentColor"
              className="cursor-pointer hover:text-[#ffad3d]"
              strokeWidth={1.8}
            />
            <HugeiconsIcon
              icon={User}
      ...
      <section className="relative overflow-hidden">
        ...
        <div
          ...
          style={{
            backgroundImage:
              "url('https://images.unsplash.com/photo-1508098682722-e99c43a406b2?q=80&w=1800&auto=format&fit=crop')",
        ...
        <div className="relative z-20 mx-auto max-w-[1400px] px-5 py-16 md:py-24 lg:py-32">
          <h1 className="text-5xl font-black uppercase leading-[0.95] tracking-tight md:text-7xl lg:text-8xl">
            ...
            One Legacy.
            <br />
            <span className="text-[#ffad3d]">One Future.</span>
          </h1>
          <p className="mt-4 text-sm font-bold uppercase tracking-wider text-white/60">
            Paro FC — Bhutan&apos;s Champions
          </p>
            ...
            {[2019, 2020, 2021, 2022, 2023].map((yr) => (
              <div key={yr} className="text-center">
                <HugeiconsIcon
                  ...
                  size={24}
                  primaryColor="currentColor"
                  className="mx-auto text-[#ffad3d]"
                  strokeWidth={1.8}
                />
                <span className="mt-1 block text-2xs font-bold text-[#ffad3d]">
                  {yr}
                </span>
              </div>
            ))}
      ...
      <section className="mx-auto max-w-[1400px] px-5 py-6">
        <div className="rounded-lg border border-[#ffad3d]/25 bg-[#141414] px-6 py-5">
          {/* Top labels */}
          <div className="mb-5">
            <h2 className="text-lg font-black uppercase text-[#ffad3d]">
              Next Match
            </h2>
          ...
          <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
            ...
            <div className="flex items-center gap-8">
              <div className="text-center">
                <Crest size="lg" />
                <p className="mt-2 text-base font-black uppercase tracking-wider">
                  Paro FC
                </p>
              </div>
        ...
        <SectionCard className="p-5">
          <div className="mb-4 flex items-end justify-between">
            <div>
              ...
              <p className="text-2xs font-bold uppercase tracking-wider text-white/40">
                Bhutan Premier League 2024/25
              </p>
            </div>
            <a
              className="text-xs font-bold uppercase tracking-wider text-[#ffad3d] hover:underline"
              href="#"
            >
              View Full Table →
            </a>
              ...
              <tbody>
                {standings.map((row) => {
                  ...
                  return (
                    <tr
                      key={row[1]}
                      className={`relative border-b border-white/5 transition ${isParo ? "bg-[#ffad3d]/10" : "hover:bg-white/[0.03]"}`}
                    >
                      ...
                      <td className="px-2 py-3">
                        <div className="flex items-center gap-2">
                          <Crest size="sm" />
                          <span
                            className={`font-black uppercase ${isParo ? "text-[#ffad3d]" : ""}`}
                          >
                            {row[1]}
                          </span>
                        </div>
                      ...
                      {row.slice(2, 10).map((v, i) => (
                        <td
                          key={i}
                          className={`px-2 py-3 text-center font-bold ${i === 7 ? (isParo ? "text-lg text-[#ffad3d]" : "text-lg") : "text-white/70"}`}
                        >
                          {v}
                        </td>
                      ))}
        ...
        <div className="flex flex-col gap-5">
          ...
          <SectionCard className="p-4">
              ...
              {raceToTitle.map((t) => {
                ...
                return (
                  <div key={t.pos} className="flex items-center gap-3">
                    ...
                    <div
                      className={`flex-1 rounded-md border px-3 py-2.5 ${isParo ? "border-[#ffad3d]/30 bg-gradient-to-r from-[#ffad3d]/15 to-transparent" : "border-white/10"}`}
                    >
                      <div className="flex items-center gap-2">
                        <Crest size="sm" />
                        <div className="flex-1">
                          <p
                            className={`text-xs font-black uppercase ${isParo ? "text-[#ffad3d]" : ""}`}
                          >
                            {t.name}
                          </p>
                          <p className={`text-base font-black ${t.color}`}>
                            {t.pts}{" "}
                      ...
                      <div className="mt-2 h-[2px] rounded-full bg-white/10">
                        <div
                          className={`h-full rounded-full ${barColor} ${barWidth}`}
                        />
                      </div>
                    </div>
                  </div>
            ...
            <div className="mt-4 grid grid-cols-3 text-center">
              <div className="border-r border-white/10">
                <p className="text-3xs font-bold uppercase tracking-wider text-white/40">
                  Gap
                </p>
                <div className="flex items-baseline justify-center">
                  <b className="text-lg text-[#ffad3d]">+5</b>
                  <span className="ml-0.5 text-2xs font-bold text-white/40">
                    PTS
                  </span>
                </div>
              </div>
          ...
          <SectionCard className="relative overflow-hidden">
            ...
            <div className="relative z-10 p-5">
              ...
              <h4 className="mt-4 text-xl font-black uppercase">
                Kinley Dorji
              </h4>
              <p className="text-xs font-bold uppercase tracking-wider text-white/40">
                Paro FC
        ...
        <SectionCard className="p-6">
          ...
          <div className="grid gap-5 md:grid-cols-3">
            {news.map(([title, date, img]) => (
              <article
                key={title}
                className="group cursor-pointer overflow-hidden rounded-md border border-[#ffad3d]/20 bg-[#0e0e0e]"
              >
                ...
                <div className="p-5">
                  ...
                  <a
                    className="mt-4 inline-flex items-center gap-1.5 text-xs font-black uppercase tracking-wider text-[#ffad3d]"
                    href="#"
                  >
                    Read More{" "}
                    <HugeiconsIcon
      ...
      <section className="mx-auto max-w-[1400px] px-5 py-8">
        <SectionCard className="p-5">
          <div className="mb-5 flex items-center justify-between">
            ...
            <a
              className="rounded-lg border border-[#ffad3d]/30 px-4 py-2 text-2xs font-black uppercase tracking-wider text-[#ffad3d] transition hover:bg-[#ffad3d]/10"
              href="#"
            >
              View All Videos →
            </a>
            ...
            {videos.map((v) => (
              <div key={v.title} className="group cursor-pointer">
                <div className="relative aspect-video overflow-hidden rounded-md bg-gradient-to-br from-[#ce0505]/30 to-[#1a0a00]">
                  ...
                  <span className="absolute bottom-2 right-2 rounded bg-black/70 px-1.5 py-0.5 text-2xs font-bold tabular-nums">
                    {v.duration}
                  </span>
                </div>
        ...
        <SectionCard className="p-6">
          ...
          <div className="grid grid-cols-2 gap-4 md:grid-cols-5">
            {[
              ...
              <div
                key={p.name}
                className="group flex flex-col items-center justify-between rounded-md border border-white/10 overflow-hidden"
              >
                ...
                <div className="w-full border-t border-white/5 py-2.5 text-center">
                  <span className="text-3xs font-bold uppercase tracking-widest text-white/35">
                    {p.role}
                  </span>
                </div>
              </div>
      ...
      <footer className="border-t border-[#ffad3d]/15 bg-[#0e0e0e]">
        <div className="mx-auto grid max-w-[1400px] gap-8 px-5 py-10 md:grid-cols-[1.4fr_1fr_1fr_1fr_1fr_1.2fr]">
          ...
          <div>
            ...
            <div className="mt-4 flex gap-2">
              {["FB", "IG", "YT", "X", "TT"].map((s) => (
                <span
                  key={s}
                  className="grid h-7 w-7 cursor-pointer place-items-center rounded-full border border-white/15 text-3xs font-bold text-white/50 transition hover:border-[#ffad3d]/40 hover:text-[#ffad3d]"
                >
                  {s}
                </span>
              ))}
          ...
          {/* Quick Links */}
          <div>
            <b className="text-2xs font-black uppercase tracking-wider text-[#ffad3d]">
              Quick Links
            </b>
            <div className="mt-3 space-y-2 text-xs text-white/40">
              {["Home", "Club", "Team", "Match Center", "Paro FC TV"].map(
                (l) => (
                  <a
                    key={l}
                    className="block transition hover:text-[#ffad3d]"
                    href="#"
                  >
                    {l}
                  </a>
          ...
          <div>
            ...
            <input
              className="mt-3 w-full rounded-lg border border-white/10 bg-white/5 px-3 py-2.5 text-xs text-white placeholder:text-white/30 focus:border-[#ffad3d]/40 focus:outline-none"
        ...
        <div className="border-t border-white/5">
          <div className="mx-auto flex max-w-[1400px] flex-wrap items-center justify-between gap-4 px-5 py-4 text-2xs text-white/25">
            ...
            <span
              className="text-sm italic text-[#ffad3d]/40"
